...
if(isset($_GET['buy'])){
    ...
    $getfromcart=$database->prepare("SELECT * from getfile where ID=:ID");
    $getfromcart->bindParam("ID",$_GET['buy']);
    $getfromcart->execute();
    $getfromcart=$getfromcart->fetchObject();
 $getfromcart->imageName;
 $getfromcart->price;
  $getfromcart->brand;
  $_SESSION['quantity'];
  ...
  <body>
    ...
    <?php 
               ...
               echo' <li class="list-group-item d-flex justify-content-between ">
                  <div>
                    <h6 class="my-0">'.$getfromcart->brand.'</h6>
                    <small class="text-muted">'.$getfromcart->categoryCat.' && Quantity'.$_SESSION['quantity'].'</small>
                  </div>
                  <span class="text-muted">$'.$getfromcart->price.'</span>
                </li>';
    ...
    ?>
                ...
                <li class="list-group-item d-flex justify-content-between">
                  <span>Total (USD)</span>
                  <strong>$<?php echo $getfromcart->price*$_SESSION['quantity'] ?></strong>
                </li>
              </ul>
            </div>
...
<?php 
if(isset($_POST['checkout'])){
//     $insertintocheckout=$database->prepare('INSERT INTO checkout(products,totalprice,IDSproduct) values(:products,:totalprice,:IDSproduct)');
//    $insertintocheckout->bindParam("products",$getfromcart->brand);
//    $insertintocheckout->bindParam("totalprice",$getfromcart->price);
//    $insertintocheckout->bindParam("IDSproduct",$_GET['buy']);
...
$name=$_POST['name'];
     ...
     $totalPrice=$getfromcart->price*$_SESSION['quantity'];
    ...
    // echo $allid;
      ...
      $insertintocheckout->bindParam("allItem",$getfromcart->brand);
    ...
    //   foreach($getId as $getId1){
        ...
        $updatecheckcart->bindParam("getId1",$_GET['buy']);
